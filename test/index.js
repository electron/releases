...
describe('electron-releases', () => {
  ...
  it('includes one release with the `latest` npm dist tag', () => {
    const latest = releases.filter((release) =>
  ...
  it('includes npm_package_name prop to indicate npm publish status', () => {
    releases
      .find((release) => release.version === '1.4.7')
      .npm_package_name.should.eq('electron')
    releases
      .find((release) => release.version === '1.0.0')
      .npm_package_name.should.eq('electron-prebuilt')
    releases
      .filter((release) => release.npm_package_name === 'electron')
      .length.should.be.above(62)

    // assert exact number because the days of publishing electron-prebuilt are over
    releases
      .filter((release) => release.npm_package_name === 'electron-prebuilt')
      .length.should.eq(96)
  })

  // for context, see https://electronjs.org/blog/npm-install-electron
  it('sets `electron` as `npm_package_name` for releases >=1.3.1', () => {
    const npmReleasesOfElectron = releases.filter(
      (release) => release.npm_package_name === 'electron'
    )
    npmReleasesOfElectron.length.should.be.above(0)
    npmReleasesOfElectron
      .every((release) => semver.gte(release.version, '1.3.1'))
      .should.eq(true)
  })

  it('sets `electron-nightly` as `npm_package_name` for nightly releases', () => {
    const npmReleasesOfElectronNightly = releases.filter(
      (release) => release.npm_package_name === 'electron-nightly'
    )
    npmReleasesOfElectronNightly.length.should.be.above(0)
  })

  it('sets `electron-prebuilt` as `npm_package_name` for releases <1.3.1', () => {
    const npmReleasesOfElectronPrebuilt = releases.filter(
      (release) => release.npm_package_name === 'electron-prebuilt'
    )
    npmReleasesOfElectronPrebuilt.length.should.be.above(0)
    npmReleasesOfElectronPrebuilt
      .every((release) => semver.lt(release.version, '1.3.1'))
      .should.eq(true)
  })
  ...
  it('includes chrome, node, and v8 versions for all recent releases', () => {
    const recentReleases = releases.filter((release) =>
      semver.gte(release.version, '1.8.0')
    )
  ...
  it('replaces all PR references like `#123` with hyperlinks', () => {
    const release1 = releases.find(
      (release) => release.version === '3.0.0-beta.1'
    )
    release1.body_html.should.contain(
      '<a href="https://github.com/electron/electron/pull/13039">#13039</a>'
    )
    release1.body_html.should.contain(
      '<a href="https://github.com/electron/electron/pull/12837">#12837</a>'
    )

    const release2 = releases.find((release) => release.version === '2.0.0')
    release2.body_html.should.contain(
      '<a href="https://github.com/electron/electron/pull/12229">#12229</a>'
    )
    release2.body_html.should.contain(
...
describe('electron-releases/lite.json', () => {
  ...
  it('includes expected properties', () => {
    const recentReleases = lite.filter((release) =>
      semver.gte(release.version, '1.8.0')
    )
    recentReleases.length.should.be.above(5)
    recentReleases
